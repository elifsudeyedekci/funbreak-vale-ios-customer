...
class _HomeScreenState extends State<HomeScreen> with SingleTickerProviderStateMixin {
  ...
  // PROVİZYON SİSTEMİ - 2000TL OTOM ATİK ÇEKİM VE GÖSTERİM!
  final double _provisionAmount = 2000.0; // Sabit provizyon tutarı
  bool _provisionProcessed = false; // Provizyon işlemi yapıldı mı?
    ...
    try {
      ...
      if (placemarks.isNotEmpty) {
        ...
        setState(() {
          ...
          // APT NUMARASI (sokak sonunda)
          if (place.subThoroughfare != null && place.subThoroughfare!.isNotEmpty) {
            addressParts.add('No: ${place.subThoroughfare}');
          }
          
          // MAHALLE (apt numarasından sonra)
  ...
  void _callValet() async {
    ...
    // KRİTİK: BEKLEYEN ÖDEME KONTROLÜ - ÖNCELİK!
    final authProvider = Provider.of<AuthProvider>(context, listen: false);
    await authProvider.checkPendingPayments();
    
    if (authProvider.hasPendingPayment) {
      print('⚠️ BEKLEYEN ÖDEME TESPIT EDİLDİ: ₺${authProvider.pendingPaymentAmount}');
      _showPendingPaymentWarning();
      return; // Vale çağırma engellendi!
    }
    
    print('✅ Bekleyen ödeme kontrolü başarılı - devam edilebilir');
              ...
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    // ÖDEME YÖNTEMİ - KOMPAKT
                    _buildCompactPaymentMethodCard(themeProvider, setModalState),
                    const SizedBox(height: 12),
                    
                    // İNDİRİM KODU - KOMPAKT
                    _buildCompactDiscountCodeCard(themeProvider, setModalState),
                    const SizedBox(height: 12),
                    ...
                    // VALE SEÇ BUTONU (2. AŞAMADA!)
                    SizedBox(
                      width: double.infinity,
                      height: 48,
                      child: ElevatedButton(
                        onPressed: (_termsAccepted && !_isLongTermTrip()) ? () {
                          Navigator.pop(context); // 2. aşamayı kapat
                          _showDriverSelectionScreen(); // Vale seç ekranını aç
                        } : null,
                        style: ElevatedButton.styleFrom(
                          backgroundColor: (_termsAccepted && !_isLongTermTrip()) ? Colors.white : Colors.grey[300],
                          foregroundColor: (_termsAccepted && !_isLongTermTrip()) ? const Color(0xFFFFD700) : Colors.grey[600],
                          side: BorderSide(
                            color: (_termsAccepted && !_isLongTermTrip()) ? const Color(0xFFFFD700) : Colors.grey[400]!,
                            width: 2,
                          ),
                          shape: RoundedRectangleBorder(
                            borderRadius: BorderRadius.circular(16),
                      ...
                      child: ElevatedButton(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            const SizedBox(width: 12),
                            Text(
                              _selectedServiceType == 'vale' ? 'Otomatik Vale Çağır' : 'Saatlik Paket Al',
                              style: const TextStyle(
                                fontSize: 18,
                                fontWeight: FontWeight.bold,
                              ),
      ...
      builder: (context) => Container(
          ...
          child: Column(
            ...
            Padding(
              ...
              child: ElevatedButton(
                  onPressed: () {
                    // VALE ARAMA İPTAL ET - ARKA PLAN ARAMASI DURDUR!
                    print('❌ VALE ARAMA İPTAL EDİLİYOR...');
                    ...
                    print('✅ Vale arama başarıyla iptal edildi!');
                  },
                style: ElevatedButton.styleFrom(
                    backgroundColor: Colors.red,
                  foregroundColor: Colors.white,
  ...
  void _simulateValetSearch(BuildContext modalContext) async {
    try {
      ...
      // 2. GERÇEK YOLCULUK TALEBİ OLUŞTUR - PANEL API'SİNE GÖNDER!
      await _createAutomaticRideRequest(); // Panel API'sine gönder
        ...
        if (!_searchCancelled && modalContext.mounted) {
          try {
            ...
            try {
              ...
              print('🚫 Vale bulunamadı - aktif talep + provizyon iade ediliyor...');
              
              // ÖNCE PROVİZYON İADE (Eğer çekilmişse)
              if (_provisionProcessed) {
                try {
                  final provisionRefund = await adminApi.processProvision(
                    customerId: customerId,
                    rideId: 'timeout_${DateTime.now().millisecondsSinceEpoch}',
                    provisionAmount: _provisionAmount,
                    action: 'refund', // İade et
                  );
                  
                  if (provisionRefund['success'] == true) {
                    print('✅ Vale bulunamadı - provizyon iade edildi');
                  } else {
                    print('⚠️ Vale bulunamadı - provizyon iade edilemedi: ${provisionRefund['message']}');
                  }
                } catch (provisionError) {
                  print('❌ Provizyon iade hatası: $provisionError');
                }
              }
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (data['success'] == true && (data['status'] == 'accepted' || data['status'] == 'confirmed')) {
            ...
            // VALE KABUL ETTİ - BACKEND PROVİZYONUN BAŞARILI OLUP OLMADIĞINI KONTROL ET!
            try {
              final rideIdFromApi = realRideDetails['ride_id']!;
              
              print('💳 Vale kabul etti - provizyon durumu kontrol ediliyor: Ride ID $rideIdFromApi');
              
              // PROVİZYON GEÇİCİ DEAKTİF - DİREKT BYPASS!
              print('💳 PROVİZYON GEÇİCİ DEAKTİF - Backend\'de zaten bypass edildi');
              bool provisionSuccess = true; // GEÇİCİ BYPASS!
  ...
  void _createRideRequest() async {
    ...
    try {
      ...
      if (user != null) {
        if (_selectedServiceType == 'vale') {
          // Normal vale talebi - ÖNCE PROVİZYON!
          print('🚗 Normal vale talebi - provizyon sistemi kontrolü...');
          
          final result = await adminApi.createRideRequest(
            customerId: user['id'],
            ...
            destinationLat: _destinationLocation!.latitude,
            destinationLng: _destinationLocation!.longitude,
            scheduledTime: _selectedDateTime ?? DateTime.now().add(const Duration(minutes: 30)),
            estimatedPrice: (_estimatedPrice ?? 0.0) - _discountAmount,
            paymentMethod: 'cash',
          );
          
          if (result['success'] == true) {
            print('✅ Normal vale talebi başarıyla oluşturuldu: ${result['ride_id']}');
            
            // VALE TALEBİ OLUŞTURDUKTAN SONRA PROVİZYON SİSTEMİNE HAZIR!
            // Provizyon vale kabul ettiğinde çekilecek (accepted status)
          } else {
            print('❌ Normal vale talebi oluşturulamadı: ${result['message']}');
          }
        } else if (_selectedServiceType == 'hourly' && _selectedHourlyPackage != null) {
          // Saatlik vale talebi - PROVİZYON SİSTEMİ İLE!
          print('⏰ Saatlik vale talebi - provizyon sistemi kontrolü...');
          
          final result = await _createHourlyRideRequest(user['id']);
          
          if (result['success'] == true) {
            print('✅ Saatlik vale talebi başarıyla oluşturuldu: ${result['ride_id']}');
            
            // SAATLİK VALE TALEBİ OLUŞTURDUKTAN SONRA PROVİZYON SİSTEMİNE HAZIR!
            // Provizyon vale kabul ettiğinde çekilecek (accepted status)
          } else {
  ...
  Future<Map<String, dynamic>> _createHourlyRideRequest(String customerId) async {
    try {
      final response = await http.post(
        Uri.parse('https://admin.funbreakvale.com/api/create_hourly_ride.php'),
        headers: {'Content-Type': 'application/json'},
        body: json.encode({
          'customer_id': customerId,
          ...
          'hourly_package_id': _selectedHourlyPackage!.id,
          'package_hours': '${_selectedHourlyPackage!.minHours.toInt()}-${_selectedHourlyPackage!.maxHours.toInt()} saat',
          'scheduled_time': (_selectedDateTime ?? DateTime.now().add(const Duration(minutes: 30))).toIso8601String(),
  ...
  void _finalizeValeCall() async {
    print('🎉 === 2. AŞAMA TAMAMLANDI - PROVİZYON VE VALE ÇAĞIRMA ===');
    ...
    try {
      ...
      // 1. ADIM: SADECE TALEP OLUŞTUR - PROVİZYON VALE KABUL EDİNCE!
      print('🚀 Vale talebi oluşturuluyor - provizyon vale KABUL EDİNCE çekilecek!');
      
      // 2. ADIM: RIDE TALEBİ OLUŞTUR - PROVİZYON DEĞİL!
  ...
  Future<void> _createAutomaticRideRequest() async {
    try {
      ...
      if (_pickupLocation == null || _destinationLocation == null) {
        ...
        // Konum bilgileri eksik ise loading'i kapat
        setState(() {
          _isLoading = false;
        });
        return;
      ...
      print('🚗 === OTOMATIK RIDE TALEBİ OLUŞTURULUYOR ===');
      ...
      // ZAMAN BAZLI RİDE OLUŞTUR
      DateTime? scheduledDateTime;
      
      if (_selectedTimeOption != 'Hemen') {
        if (_selectedDateTime != null) {
          scheduledDateTime = _selectedDateTime!;
          print('🕰️ Zamanlı talep: $_selectedDateTime');
        }
      }
      
      // Panel API ile ride talebi oluştur - Safe provider access
      AdminApiProvider? adminApi;
      try {
        adminApi = Provider.of<AdminApiProvider>(context, listen: false);
      } catch (e) {
      ...
      final result = await adminApi.createRideRequest(
        customerId: authProvider.customerId ?? '1',
        pickupAddress: _pickupAddress,
        pickupLat: _pickupLocation!.latitude,
        pickupLng: _pickupLocation!.longitude,
        destinationAddress: _destinationAddress,
        destinationLat: _destinationLocation!.latitude,
        destinationLng: _destinationLocation!.longitude,
        scheduledTime: scheduledDateTime ?? DateTime.now(),
      ...
      if (result['success'] == true) {
        ...
        print('🆔 Request ID: ${result['data']?['request_id'] ?? 'ID yok'}');
  ...
  // KAPSAMLI PROVİZYON SİSTEMİ - TÜM YOLCULUK TÜRLERİ İÇİN!
  Future<bool> _processProvision({String? rideId, String rideType = 'standard'}) async {
    print('💳 Kapsamlı provizyon başlıyor: ₺${_provisionAmount.toStringAsFixed(2)} - Tür: $rideType');
    
    try {
      // GERÇEK PROVİZYON API ENTEGRESyONU!
      final adminApi = AdminApiProvider();
      final prefs = await SharedPreferences.getInstance();
      final customerId = prefs.getString('user_id') ?? '0';
      
      // Ride ID kontrolü
      final currentRideId = rideId ?? DateTime.now().millisecondsSinceEpoch.toString();
      
      print('💳 Provizyon API çağrısı - Tür: $rideType, Ride: $currentRideId');
      
      final provisionResult = await adminApi.processProvision(
        customerId: customerId,
        rideId: currentRideId,
        provisionAmount: _provisionAmount,
        action: 'charge', // VALE BULUNDUKTAN SONRA ÇEK!
        paymentMethod: 'card',
      );
      
      if (provisionResult['success'] == true) {
        setState(() {
          _provisionProcessed = true;
        });
        
        final message = provisionResult['message'] ?? 'Provizyon alındı';
        print('✅ $rideType provizyon başarılı: $message');
        ...
        final errorMessage = provisionResult['message'] ?? 'Provizyon alınamadı';
        print('❌ $rideType provizyon başarısız: $errorMessage');
      ...
      print('❌ $rideType provizyon hatası: $e');
      
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('❌ Provizyon hatası: $e'),
            backgroundColor: Colors.red,
            duration: const Duration(seconds: 4),
          ),
        );
  ...
  Future<void> _cancelCurrentRideRequest({String reason = 'user_cancel'}) async {
    try {
      ...
      // Önce mevcut provizyon var mı kontrol et ve gerekirse iptal et
      if (_provisionProcessed) {
        print('💳 Provizyon mevcut - iptal işlemi yapılıyor...');
        
        final provisionCancel = await adminApi.processProvision(
          customerId: customerId,
          rideId: DateTime.now().millisecondsSinceEpoch.toString(), // Temp ID
          provisionAmount: _provisionAmount,
          action: 'cancel', // 45 dakika kuralı ile iptal
        );
        
        if (provisionCancel['success'] == true) {
          print('✅ Provizyon iptal işlemi başarılı: ${provisionCancel['message']}');
        }
      }
      ...
      if (cancelResult['success'] == true) {
        ...
        setState(() {
          _provisionProcessed = false; // Provizyon durumunu sıfırla
        });
      }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // TAHMİNİ FİYAT - ORIJINAL YERİNE!
          if (_originalPrice != null) ...[
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text(
                  'Tahmini Fiyat:', // Orijinal → Tahmini
                  style: TextStyle(
                    fontSize: 14, // 16 → 14
                    color: themeProvider.isDarkMode ? Colors.grey[300] : Colors.grey[700],
                  ),
          ...
          // PROVİZYON GÖSTERİMİ - YENİ ÖZELLİK!
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text(
                'Provizyon (Kartınızdan):',
                style: TextStyle(
                  fontSize: 14,
                  color: Colors.orange[700],
                  fontWeight: FontWeight.w600,
              ...
              Text(
                '₺${_provisionAmount.toStringAsFixed(2)}',
                style: TextStyle(
                  fontSize: 14,
                  color: Colors.orange[700],
                  fontWeight: FontWeight.bold,
          ...
          // PROVİZYON AÇIKLAMA - TAM GÖRÜNÜR!
          Container(
            padding: const EdgeInsets.all(12), // 8 → 12 daha geniş
            decoration: BoxDecoration(
              color: Colors.orange.withOpacity(0.1),
            ...
            child: Row(
              ...
              children: [
                Icon(Icons.info, color: Colors.orange[700], size: 18), // 16 → 18
                const SizedBox(width: 10), // 8 → 10
                Expanded(
                  child: Text(
                    'Yolculuk başladığında otomatik olarak kartınızdan provizyon alınacaktır. Yolculuk bitiminde gerçek tutar üzerinden hesaplanacaktır.',
                    style: TextStyle(
                      fontSize: 12, // 11 → 12 daha okunaklı
                      color: Colors.orange[800],
                      height: 1.4, // Satır yüksekliği
                    ),
                    // maxLines ve overflow kaldırıldı - TAMAMINI GÖSTER!
                  ),
                ),
              ],
            ),
  ...
  Widget _buildCompactPaymentMethodCard(ThemeProvider themeProvider, StateSetter setModalState) {
    // VARSAYILAN KAYITLI KART (Kullanıcı değiştirebilir)
    final defaultCard = {
      'cardNumber': '**** **** **** 1234',
      'cardType': 'visa',
      'isDefault': true
        ...
        child: Row(
          children: [
            Container(
              ...
              height: 20,
              decoration: BoxDecoration(
                color: _getCardTypeColor(defaultCard['cardType'].toString()),
                borderRadius: BorderRadius.circular(4),
              ),
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Text(
                'Ön Bilgilendirme Koşulları ve Mesafeli Satış Sözleşmesi\'ni onaylıyorum.',
                style: TextStyle(
                  fontSize: 12, // 14 → 12
                  color: themeProvider.isDarkMode ? Colors.grey[300] : Colors.grey[700],
                ),
...
Kabul etmekle bu şartları onaylamış bulunmaktasınız.
  ...
  // SPESİFİK VALE ÇAĞIRMA İŞLEMİ - PROVİZYON VALE KABUL EDİNCE!
  Future<void> _processSpecificDriverCall(Map<String, dynamic> driver) async {
    ...
    print('🚀 MANUEL VALE - provizyon vale KABUL EDİNCE çekilecek!');
    
    try {
      // SADECE VALE ÇAĞIR - PROVİZYON KABUL EDİNCE!
