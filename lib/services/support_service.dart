...
class SupportService {
    ...
    try {
      ...
      if (_cachedSupport != null && _lastFetch != null) {
        if (DateTime.now().difference(_lastFetch!) < cacheTimeout) {
          print('Support info cache\'den alındı');
          return _cachedSupport!;
        }
      }
      
      print('Support info API\'den çekiliyor...');
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          String appName = settings['app_name'] ?? 'FunBreak Vale';
          ...
          print('Support info güncellendi: $phone, $email');
          return _cachedSupport!;
        }
      }
      
      print('Support info alınamadı, varsayılan değerler kullanılıyor');
      return _getDefaultSupportInfo();
      
    } catch (e) {
      print('Support info hatası: $e');
      return _getDefaultSupportInfo();
    }
  }
  ...
  static Future<String> getAppName() async {
    final info = await getSupportInfo();
    return info['app_name'] ?? 'FunBreak Vale';
  }
  ...
  static void clearCache() {
    _cachedSupport = null;
    _lastFetch = null;
    print('Support info cache temizlendi');
  }
